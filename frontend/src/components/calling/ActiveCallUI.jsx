...
const ActiveCallUI = () => {
  ...
  // Make audio refs globally accessible for WebRTC hook
  useEffect(() => {
    if (typeof window !== 'undefined') {
      window.__webrtc_local_audio = localAudioRef.current;
      window.__webrtc_remote_audio = remoteAudioRef.current;
      console.log('🔊 Audio elements registered:', {
  ...
  return (
    <>
      {/* Hidden audio elements - ALWAYS rendered so WebRTC can access them */}
      ...
      <audio
        ...
        style={{ display: 'none' }}
      />
      {/* Remote audio (plays incoming audio) */}
      <audio
        ref={remoteAudioRef}
        id="remote-audio"
        autoPlay
      ...
      {/* Only show UI when call is active */}
      {isActive && (
        <AnimatePresence>
          <motion.div
          ...
          >
            <div className="relative">
              {/* Glow effect */}
              <div className={`absolute inset-0 rounded-3xl blur-xl opacity-40 ${isConnected ? 'bg-emerald-500' : 'bg-amber-500'
                }`} />
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="relative">
                    <motion.div
                      ...
                      className={`w-14 h-14 rounded-full flex items-center justify-center ${isConnected
                    ...
                    <motion.div
                      ...
                      className={`absolute -bottom-1 -right-1 w-5 h-5 rounded-full flex items-center justify-center ${isConnected ? 'bg-emerald-500' : 'bg-amber-500'
                        }`}
                    >
                      {isConnecting ? (
